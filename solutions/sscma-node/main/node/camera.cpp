...
ma_err_t CameraNode::onCreate(const json& config) {
    ...
    capture_ = new cv::VideoCapture(pipeline);
    ...
    while (started_) {
        if (capture_->read(frame)) {
            ...
            if (preview_) {
                ...
                cv::resize(frame, preview, cv::Size(320, 240), 0, 0, cv::INTER_LINEAR);
                std::vector<int> params_ = {cv::IMWRITE_JPEG_QUALITY, 50};
                cv::imencode(".jpg", preview, buffer_, params_);
                // convert to base64
